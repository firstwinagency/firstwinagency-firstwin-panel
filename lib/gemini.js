...
async function refToBase64(ref) {
  ...
  if (!res.ok) throw new Error(`No se pudo descargar la imagen: ${ref}`);
...
async function generatePro(prompt, refs) {
  ...
  if (!res.ok) {
    throw new Error(`Imagen Pro Error: ${JSON.stringify(json)}`);
  }

  const img = json?.images?.[0];
  if (!img?.base64) throw new Error("Pro: no se generó imagen");
